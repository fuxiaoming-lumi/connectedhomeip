...
OnOffLightManager OnOffLightManager::sLight;
namespace {
...
OnOffEffect gEffect = {
    chip::EndpointId{ 1 },
    OnOffLightMgr().OnTriggerOffWithEffect,
...
CHIP_ERROR OnOffLightManager::Init()
{
...
void OnOffLightManager::SetCallbacks(Callback_fn_initiated aActionInitiated_CB, Callback_fn_completed aActionCompleted_CB)
{
    mActionInitiated_CB = aActionInitiated_CB;
    mActionCompleted_CB = aActionCompleted_CB;
}

bool OnOffLightManager::IsActionInProgress()
{
    return (mState == kState_OffInitiated || mState == kState_OnInitiated);
}

bool OnOffLightManager::IsLightOn()
{
    return (mState == kState_OnCompleted);
}

void OnOffLightManager::EnableAutoTurnOff(bool aOn)
{
    mAutoTurnOff = aOn;
}

void OnOffLightManager::SetAutoTurnOffDuration(uint32_t aDurationInSecs)
{
    mAutoTurnOffDuration = aDurationInSecs;
}

bool OnOffLightManager::InitiateAction(int32_t aActor, Action_t aAction)
{
...
void OnOffLightManager::StartTimer(uint32_t aTimeoutMs)
{
...
void OnOffLightManager::CancelTimer(void)
{
...
void OnOffLightManager::TimerEventHandler(void * timerCbArg)
{
    // The callback argument is the light obj context assigned at timer creation.
    OnOffLightManager * light = static_cast<OnOffLightManager *>(timerCbArg);
...
void OnOffLightManager::AutoTurnOffTimerEventHandler(AppEvent * aEvent)
{
    OnOffLightManager * light = static_cast<OnOffLightManager *>(aEvent->TimerEvent.Context);
...
void OnOffLightManager::OffEffectTimerEventHandler(AppEvent * aEvent)
{
    OnOffLightManager * light = static_cast<OnOffLightManager *>(aEvent->TimerEvent.Context);
    int32_t actor           = AppEvent::kEventType_Timer;
...
void OnOffLightManager::ActuatorMovementTimerEventHandler(AppEvent * aEvent)
{
...
void OnOffLightManager::OnTriggerOffWithEffect(OnOffEffect * effect)
{
    ...
    OnOffLightMgr().mOffEffectArmed = true;
    OnOffLightMgr().StartTimer(offEffectDuration);
}
